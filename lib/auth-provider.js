...
function resolvePortalAccess(roleNames, sourceTeam, config) {
  ...
  // 1. Assign Connect Admin access strictly to users assigned service.admin role in database
  if ([...roles].some(role => config.adminRoles.has(role))) {
    return { role: 'admin', team: null };
  }

  // 2. Explicitly deny Connect access for Super Admin roles (unless assigned service.admin in database)
